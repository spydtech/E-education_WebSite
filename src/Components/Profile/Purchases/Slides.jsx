...
function Slides() {
  ...
  return (
    <div className='p-10'>
          ...
          {cards.map((card, index) => (
            <div key={index} className="p-4">
              <div className="bg-white rounded-lg shadow-lg p-6 h-96">
                ...
                <a
                  className="text-sky-500 transition-all duration-300 group-hover:text-white cursor-pointer"
                >
                  Explore &rarr;
                </a>
              </div>
      ...
      <div className="flex h-screen items-center justify-center bg-blue-100 p-5">
        <div className="grid md:grid-cols-2 grid-cols-1 items-center gap-10 md:px-10">
          <div>
            <img src={IMG} alt="Education Logo" />
            <h1 className="mb-2 text-3xl font-bold">
              <span className="text-green-500">Try out different courses to see which one fits your needs</span>
            </h1>
            <p className="mb-6">
              Get a 7-day free trial that includes courses, Specializations, Projects, and Professional Certificates.
            </p>
            <div className="flex justify-center space-x-5">
              <button className="flex w-full items-center justify-center gap-1 rounded-2xl bg-blue-500 p-5 py-3 font-semibold text-white hover:bg-blue-700">
                Learn More &rarr;
              </button>
            </div>
          </div>
          <div>
            <img src="https://intellipaat.com/academy/wp-content/uploads/2022/03/home-banner.png" alt="Banner" className="md:size-96 size-72 rounded-full " />
          </div>
        </div>
      </div>
    </div>
