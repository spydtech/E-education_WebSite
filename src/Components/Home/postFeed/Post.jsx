...
import Avatar from '@mui/material/Avatar';
import { CiCamera } from "react-icons/ci";
import { FaRegPenToSquare, FaRegThumbsUp, FaThumbsUp, FaRegMessage, FaTrash } from "react-icons/fa6";
...
export default function Post() {
    ...
    const handlePost = () => {
        if (editTweetId) {
            setTweets(tweets.map(tweet => tweet.id === editTweetId ? { ...tweetData, id: editTweetId } : tweet));
            setEditTweetId(null);
        } else {
            setTweets([tweetData, ...tweets]);
        }
    ...
    return (
        <div className="mx-40">
            ...
            <div className="bg-white mt-3 overflow-y-auto h-[600px] overflow-hidden ">
                    ...
                    <div className="p-4 border border-gray-300 rounded-lg mt-4" key={tweet.id}>
                        <div className="flex justify-between">
                            ...
                            <div className="ml-auto flex items-center gap-2">
                                <FaEdit className="text-blue-600 cursor-pointer" onClick={() => editTweet(tweet)} />
                                <FaTrash className="text-red-600 cursor-pointer" onClick={() => deleteTweet(tweet.id)} />
                            </div>
                        </div>
                        <div className="mt-2">{tweet.content}</div>
                        {tweet.image && <img src={tweet.image} alt="Tweet" className="mt-2 max-w-full h-auto" />}
                        <div className="mt-2 flex justify-start items-center  border-t border-gray-300 gap-8 py-4">
                            <div className="flex justify-center items-center flex-row gap-2 text-[#0098F1] mr-4 cursor-pointer" onClick={() => toggleLike(tweet.id)}>
                              <div> {tweet.isLiked ? <FaThumbsUp className="text-blue-600" /> : <FaRegThumbsUp />}</div>  
                        ...
                        <div className="mt-2">
                            {tweet.comments && tweet.comments.map((comment) => (
                                <div key={comment.id} className="flex items-center">
                                    ...
                                    <div className="bg-gray-100 p-2 rounded-lg">{comment.text}</div>
                                </div>
                            ))}
                        </div>
                    </div>
