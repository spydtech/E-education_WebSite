...
const Card = ({ id, data, currentId, setCurrentId }) => {
  const changeId = (id) => {
    let out = "h-[250px] w-[500px] rounded-md shadow-lg ";
    if (id === currentId) {
      out +=
        "bg-[#4cc9f0] text-white absolute md:-top-[50px] md:scale-[1.12] -top-[25px] left-[50%] -translate-x-[50%] z-20 ";
      ...
      <div className="flex items-center relative">
        <div>
          <img
            src={data.image_url}
            alt="person-img"
            className="m-5 h-[50px] rounded-full"
          ></img>
        </div>
        ...
        <div className="flex items-center gap-1 lg:text-lg text-sm absolute lg:right-10 right-6">
          <FaStar
            className={data.rating >= 1 ? " text-[#edd661] " : "text-gray-400"}
          />
          <FaStar
            className={data.rating >= 2 ? " text-[#edd661] " : "text-gray-400"}
          />
          <FaStar
            className={data.rating >= 3 ? " text-[#edd661] " : "text-gray-400"}
          />
          <FaStar
            className={data.rating >= 4 ? " text-[#edd661] " : "text-gray-400"}
          />
          <FaStar
            className={data.rating >= 5 ? " text-[#edd661] " : "text-gray-400"}
          />
          <div>({data.rating})</div>
        </div>
      </div>
