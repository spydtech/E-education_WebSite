import React from "react";

const RegressionTestingCard = () => {
  return (
    <>
      <section id="portfolio" className="portfolio-section py-16 px-4">
        <div className="container mx-auto">
          <div className=" mb-12">
            <h2 className="text-4xl font-bold">
              Mastering Regression Testing: Maintain Stability and Quality
            </h2>
            <p className="text-lg text-[#00a8e8] font-semibold mt-2">
              Enhance your expertise with our Regression Testing courses. Dive
              deep into methodologies and gain hands-on experience. Whether
              novice or pro, ensure your applications remain stable and
              bug-free.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            <div className="group portfolio-item relative hover:shadow-lg shadow-md rounded-lg overflow-hidden">
              <a>
                <img
                  className="w-full h-60 object-cover"
                  src="https://img.freepik.com/free-vector/illustration-friends-videoconferencing-scene_23-2148831742.jpg?t=st=1716955432~exp=1716959032~hmac=c8dc0b31e13fbf40d359cb4ab4883eaa492a0fe37e3ff46284fbaace7cd62ffc&w=826"
                  alt="Regression Testing Introduction"
                />
                <div className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-r from-[#6f1d1b] to-[#432818] opacity-0 transition duration-300 ease-in-out group-hover:opacity-70"></div>
                <div className="p-4 flex flex-col items-center justify-between relative z-10">
                  <h3 className="text-lg font-medium text-txt group-hover:text-gray-dark">
                    Explore Course Catalog
                  </h3>
                  <span className="text-sm font-bold text-[#00a8e8] group-hover:text-white pt-4">
                    Browse our extensive catalog of Regression Testing courses.
                    Discover beginner-friendly to advanced-level programs
                    designed to accommodate learners of all backgrounds.
                  </span>
                </div>
              </a>
            </div>
            <div className="group portfolio-item relative hover:shadow-lg shadow-md rounded-lg overflow-hidden">
              <a>
                <img
                  className="w-full h-60 object-cover"
                  src="https://img.freepik.com/free-vector/design-inspiration-concept-illustration_114360-4191.jpg?t=st=1716955482~exp=1716959082~hmac=1a333d32c61cf6e4ef49d679bc0280833bcaeb3f33f69892010e8030501f749c&w=740"
                  alt="Regression Testing Foundations"
                />
                <div className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-r from-[#6f1d1b] to-[#432818] opacity-0 transition duration-300 ease-in-out group-hover:opacity-70"></div>
                <div className="p-4 flex flex-col items-center justify-between relative z-10">
                  <h3 className="text-lg font-medium text-txt group-hover:text-gray-dark">
                    Enroll in Foundational Courses
                  </h3>
                  <span className="text-sm font-bold text-[#00a8e8] group-hover:text-white pt-4">
                    Start with foundational courses covering basic concepts,
                    tools, and methodologies essential for Regression Testing.
                  </span>
                </div>
              </a>
            </div>
            <div className="group portfolio-item relative hover:shadow-lg shadow-md rounded-lg overflow-hidden">
              <a>
                <img
                  className="w-full h-60 object-cover"
                  src="https://img.freepik.com/premium-vector/flat-illustration-design-analyzing_9206-2760.jpg?w=740"
                  alt="Regression Testing Projects"
                />
                <div className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-r from-[#6f1d1b] to-[#432818] opacity-0 transition duration-300 ease-in-out group-hover:opacity-70"></div>
                <div className="p-4 flex flex-col items-center justify-between relative z-10">
                  <h3 className="text-lg font-medium text-txt group-hover:text-gray-dark">
                    Practice with Real-world Projects
                  </h3>
                  <span className="text-sm font-bold text-[#00a8e8] group-hover:text-white pt-4">
                    Apply your knowledge by working on hands-on projects
                    tailored to real-world scenarios. Gain practical experience
                    in Regression Testing, defect tracking, and test case
                    execution under the guidance of industry experts.
                  </span>
                </div>
              </a>
            </div>
            <div className="group portfolio-item relative hover:shadow-lg shadow-md rounded-lg overflow-hidden">
              <a>
                <img
                  className="w-full h-60 object-cover"
                  src="https://img.freepik.com/premium-vector/designer-secretary-man-with-laptop-programmer-tester-digital-artist-student-smm-remote-work_909058-1037.jpg?w=740"
                  alt="Track Your Progress"
                />
                <div className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-r from-[#6f1d1b] to-[#432818] opacity-0 transition duration-300 ease-in-out group-hover:opacity-70"></div>
                <div className="p-4 flex flex-col items-center justify-between relative z-10">
                  <h3 className="text-lg font-medium text-txt group-hover:text-gray-dark">
                    Track Your Progress
                  </h3>
                  <span className="text-sm font-bold text-[#00a8e8] group-hover:text-white pt-4">
                    Monitor your progress with intuitive dashboards and tracking
                    tools. Set goals, track milestones, and measure your
                    proficiency in various Regression Testing concepts as you
                    advance through the courses.
                  </span>
                </div>
              </a>
            </div>
            <div className="group portfolio-item relative hover:shadow-lg shadow-md rounded-lg overflow-hidden">
              <a>
                <img
                  className="w-full h-60 object-cover"
                  src="https://img.freepik.com/premium-vector/it-consultant-quality-control-service-programming-code-testing-software-developer-writing-abstract-metaphor_335657-2263.jpg?w=740"
                  alt="Specialize and Advance"
                />
                <div className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-r from-[#6f1d1b] to-[#432818] opacity-0 transition duration-300 ease-in-out group-hover:opacity-70"></div>
                <div className="p-4 flex flex-col items-center justify-between relative z-10">
                  <h3 className="text-lg font-medium text-txt group-hover:text-gray-dark">
                    Specialize and Advance
                  </h3>
                  <span className="text-sm font-bold text-[#00a8e8] group-hover:text-white pt-4">
                    Choose specialized tracks or advanced courses to delve
                    deeper into specific areas of Regression Testing, such as
                    exploratory testing, usability testing, or mobile testing.
                    Customize your learning path to align with your interests
                    and career goals.
                  </span>
                </div>
              </a>
            </div>
            <div className="group portfolio-item relative hover:shadow-lg shadow-md rounded-lg overflow-hidden">
              <a>
                <img
                  className="w-full h-60 object-cover"
                  src="https://img.freepik.com/premium-vector/quality-control-concept-illustration_701961-1878.jpg?w=740"
                  alt="Earn Recognized Certifications"
                />
                <div className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-r from-[#6f1d1b] to-[#432818] opacity-0 transition duration-300 ease-in-out group-hover:opacity-70"></div>
                <div className="p-4 flex flex-col items-center justify-between relative z-10">
                  <h3 className="text-lg font-medium text-txt group-hover:text-gray-dark">
                    Earn Recognized Certifications
                  </h3>
                  <span className="text-sm font-bold text-[#00a8e8] group-hover:text-white pt-4">
                    Upon successful completion of courses and projects, earn
                    industry-recognized certifications to showcase your
                    expertise to employers and enhance your career prospects.
                  </span>
                </div>
              </a>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default RegressionTestingCard;
